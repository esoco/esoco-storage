	...
	extends AbstractStorageMapping<T, FieldDescriptor, ClassMapping<?>>
{
	 ...
	 *
	 * <p>To be usable for all storage implementations the given class must have
	 * a public constructor without parameters so that new objects can be
	...
	public T createObject(List<?> rAttributeValues, boolean bAsChild)
	{
		...
		{
			throw new IllegalStateException("Could not create instance of " +
											rMappedType,
											e);
		}
	}
		...
		throws StorageException
	{
		...
		if (rValue != null && rField.hasRelation(STORAGE_MAPPING))
		{
			...
			StorageMapping<Object, Relatable, ?> rMapping =
				(StorageMapping<Object, Relatable, ?>) rField.get(STORAGE_MAPPING);
	 ...
	 */
	@Override
	public Collection<?> getChildren(T rObject, ClassMapping<?> rChildMapping)
	{
		return (Collection<?>) aChildFields.get(rChildMapping)
	 ...
	 */
	@Override
	public void initChildren(T				 rParent,
							 List<?>		 rChildren,
	 ...
	 */
	@Override
	public void setAttributeValue(T				  rObject,
	 ...
	 */
	@Override
	public void setChildren(T				rParent,
							List<?>			rChildren,
							ClassMapping<?> rChildMapping)
	{
		...
		for (Field rField : rMappedType.getDeclaredFields())
		{
			...
			if ((nModifiers & IGNORED_MODIFIERS) == 0)
			{
				...
				if (Collection.class.isAssignableFrom(rField.getType()))
				{
					...
					Log.debug(String.format("Child Mapping: %s,%s",
											rChildMapping,
											aFieldDescriptor));
				}
				else
				{
					...
					Log.debug(String.format("Field Mapping: %s",
											aFieldDescriptor));
				}
			}
		}
	 ...
	 *
	 * @throws IllegalArgumentException If the field cannot be mapped to a child
	 *                                  collection
	 */
	private ClassMapping<?> createChildMapping(Field rField)
	{
		...
		{
			throw new IllegalArgumentException("Could not determine element type of " +
											   rField.getName());
		}

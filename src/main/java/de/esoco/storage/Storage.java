 ...
 * @author eso
 */
public abstract class Storage extends RelatedObject implements Transactional,
															   Releasable
{
	 ...
	 * that are closed and can therefore return FALSE in such a case to indicate
	...
	{
		checkStorageDeleteEnabled(StorageManager.getMapping(rObject.getClass()));
		deleteObject(rObject);
	}

	/***************************************
	 * Checks whether this storage has already been initialized for a particular
	 * object type.
	 *
	...
	/***************************************
	 * Initializes this storage for a particular object type to allow storing or
	 * retrieving objects of this type. What effect this method has depends on
	 * the actual storage implementation. For example, a JDBC storage may create
	 ...
	 *
	 * <p>For long-term storages this method only needs to be invoked once for a
	 * particular object type. Additional invocations with the same object type
	 * will not harm but may need additional test and should therefore be
	 ...
	 *
	 * <p>Storage implementations may override this method to perform additional
	 * maintenance on a release but it is necessary that they always invoke this
	 * superclass method. In most cases it should be more appropriate to perform
	 * the cleanup in the implementation of the {@link #close()} method which is
	 * invoked by the storage management.</p>
	 *
	 ...
	 * What effect this method has depends on the actual storage implementation.
	 ...
	 * NOT remove the storages for subordinate types of the the given type if it
	 * has such. This is intentional to prevent the accidental cascaded deletion
	...
	/***************************************
	 * Must be implemented to close the storage. This method should normally not
	 ...
	 * structures for a certain storage mapping. Invocation if no object storage
	 ...
	 *
	 * @throws Exception Any exception may be thrown if storing the object fails
	 */
	...
	private void storeSingleObject(Object rObject) throws StorageException
	{
		...
		try
		{
			...
			// only reset MODIFIED flag if relation exists, i.e. object supports
